...
The demo needs no key. If `OPENAI_API_KEY` is set (optionally `OPENAI_MODEL`), the **Rewrite with LLM (optional)** button calls the server-only `/api/explain` route to reword the summary the deterministic agents already produced. It is never called automatically, the key never reaches the browser, and if it is missing or fails the offline explanation stands unchanged. No matching, scoring or reconciliation decision ever depends on an LLM.

## Why this is not a chat-invoice tool

HisabAgent does not generate invoices or put a chatbot in front of a ledger. Its moat is the inspectable multi-agent trace, confidence-banded evidence, an adversarial critic that argues with its own matcher, a human exception queue with ready-to-send owner messages, and a regression-guarded eval suite.
